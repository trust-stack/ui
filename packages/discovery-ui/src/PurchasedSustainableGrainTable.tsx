...
export function PurchasedSustainableGrainTable({
  purchasedSustainableGrains,
}: PurchasedSustainableGrainTableProps) {
  const columns = [
  ...
  const rows = useMemo(() => {
    return purchasedSustainableGrains ?? defaultData;
  }, [purchasedSustainableGrains]);
  ...
  return (
    <>
      <YStack padding={"$spacing.exp_margin"}>
        <Headline size="small">Purchased Sustainable Grains</Headline>
...
type TData = PurchasedSustainableGrain & {
  actions: string;
};
